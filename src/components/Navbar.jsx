...
import { 
  Crosshair, Trophy, User, LogOut, Menu, X, 
  ShieldCheck, LifeBuoy, Info, ChevronDown 
} from "lucide-react";
...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "Tournaments", href: "/tournaments", icon: <Trophy className="h-4 w-4" /> },
    { name: "Team Finder", href: "/team-finder", icon: <User className="h-4 w-4" /> },
    { name: "Leaderboard", href: "/leaderboard", icon: <ShieldCheck className="h-4 w-4" /> },
  ];
  ...
  return (
    <header className="fixed top-0 z-50 w-full">
      ...
      <div className="absolute inset-0 bg-slate-950/40 backdrop-blur-xl border-b border-white/5 shadow-[0_4px_30px_rgba(0,0,0,0.1)]" />
      ...
      <div className="container relative mx-auto px-4">
        <div className="flex h-20 items-center">
          ...
          <nav className="hidden lg:flex items-center gap-1">
              ...
              return (
                <Link
                  ...
                  className={`relative flex items-center gap-2 rounded-full px-5 py-2 text-sm font-semibold transition-all duration-300 ${
                    isActive 
                      ? "text-rose-500 bg-rose-500/5 shadow-[0_0_20px_rgba(244,63,94,0.1)]" 
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {/* Dropdown Menu */}
              {isMoreOpen && (
                <div className="absolute left-1/2 mt-3 w-48 -translate-x-1/2 overflow-hidden rounded-2xl border border-white/10 bg-slate-900/95 p-1.5 shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in duration-200">
                  {moreLinks.map((link) => (
                    <Link
                      key={link.name}
                      href={link.href}
          ...
          <div className="flex flex-1 items-center justify-end gap-4">
                ...
                {user ? (
                  <div className="flex items-center gap-3">
                    {isAdmin && (
                      <Link
                        href="/admin"
                        className="hidden md:flex items-center gap-2 rounded-full border border-rose-500/20 bg-rose-500/10 px-4 py-2 text-xs font-bold uppercase tracking-wider text-rose-500 transition-all hover:bg-rose-500 hover:text-white"
                      >
                    ...
                    >
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-slate-500 leading-none">View Profile</span>
                      </div>
                    </Link>
            ...
            {/* Mobile Menu Toggle */}
            <button 
              className="flex lg:hidden h-10 w-10 items-center justify-center rounded-full bg-white/5 text-slate-300 transition-colors hover:bg-white/10" 
        ...
        {/* Mobile menu content */}
        {isOpen && (
          <div className="lg:hidden absolute left-4 right-4 top-24 rounded-3xl border border-white/10 bg-slate-900/90 p-6 shadow-2xl backdrop-blur-2xl transition-all duration-300">
